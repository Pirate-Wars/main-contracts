...
contract("ARGH_Token", ([deployer, receiver, exchange]) => {
  ...
  beforeEach(async () => {
    ARGH_Token = await ARGH_Token.new(deployer);
  });

  describe("deployment", () => {
    it("tracks the name: Argh", async () => {
      const result = await ARGH_Token.name();
      result.should.equal(name);
    });

    it("tracks the symbol: ARGH", async () => {
      const result = await ARGH_Token.symbol();
      result.should.equal(symbol);
    });

    it("tracks the decimals: 18", async () => {
      const result = await ARGH_Token.decimals();
      result.toString().should.equal(decimals);
    });

    it("tracks the total supply: 0", async () => {
      const result = await ARGH_Token.totalSupply();
      result.toString().should.equal(totalSupply);
    });

    it("assigns the total supply to the deployer: 0", async () => {
      const result = await ARGH_Token.balanceOf(deployer);
      result.toString().should.equal(totalSupply);
    });
  });
  ...
  describe("sending tokens", () => {
    ...
    describe("success", async () => {
      beforeEach(async () => {
        ...
        minting = await ARGH_Token.mint(deployer, mintedAmount);
        result = await ARGH_Token.transfer(receiver, amount);
        amountBurned = "100";
        await ARGH_Token.burn(amountBurned);
      });

      it("transfers token balances", async () => {
        let balanceOf;
        balanceOf = await ARGH_Token.balanceOf(deployer);
        balanceOf.toString().should.equal(remainingAmount);
        balanceOf = await ARGH_Token.balanceOf(receiver);
    ...
    describe("failure", async () => {
      ...
      it("rejects invalid recipients", async () => {
        await ARGH_Token.transfer(0x0, amount, { from: deployer }).should.be
          .rejected;
      });
    });
  });
  ...
  describe("approving tokens", () => {
    ...
    beforeEach(async () => {
      amount = "100000000000000000000";
      result = await ARGH_Token.approve(exchange, amount, { from: deployer });
    });

    describe("success", () => {
      it("allocates an allowance for delegated token spending on exchange", async () => {
        const allowance = await ARGH_Token.allowance(deployer, exchange);
    ...
    describe("failure", () => {
      it("rejects invalid spenders", async () => {
        await ARGH_Token.approve(0x0, amount, { from: deployer }).should.be
          .rejected;
      });

      it("rejects minting more than the max supply", async () => {
        mintedAmount = "5000000";
        await ARGH_Token.mint(deployer, mintedAmount).should.be
          .rejected;
      });
    });
  });
  ...
  describe("delegated token transfers", () => {
    ...
    beforeEach(async () => {
      ...
      minting = await ARGH_Token.mint(deployer, mintedAmount);
      await ARGH_Token.approve(exchange, amount, { from: deployer });
    });
    describe("success", async () => {
      beforeEach(async () => {
        result = await ARGH_Token.transferFrom(deployer, receiver, amount, {
          from: exchange,
        });
      });
    ...
    describe("failure", async () => {
      ...
      it("rejects invalid recipients", async () => {
        await ARGH_Token.transferFrom(deployer, 0x0, amount, { from: exchange })
          .should.be.rejected;
      });
    });
  });
  ...
  describe("activating the Pause function and testing", async () => {
    beforeEach(async () => {
      ...
      minting = await ARGH_Token.mint(deployer, "500");

      await ARGH_Token.pause();
    ...
    it("should reject minting becuase its pasued", async () => {
      mintedAmount = "500";
      await ARGH_Token.mint(deployer, mintedAmount).should.be.rejected;
    });

    it("should reject transferring becuase its pasued", async () => {
      amount = "100000000000000000000";

      await ARGH_Token.transfer(receiver, amount).should.be.rejected;
    });

    it("should reject burn becuase its pasued", async () => {
      amount = "100000000000000000000";

      await ARGH_Token.burn(amount).should.be.rejected;
    });
    ...
    it("unpasue again and retest to make sure MINT works again", async () => {
      await ARGH_Token.unpause();
      mintedAmount = "100";
      await ARGH_Token.mint(deployer, mintedAmount);
    });

    it("unpasue again and retest to make sure BURN works again", async () => {
      await ARGH_Token.unpause();
      amount = "100";
      await ARGH_Token.burn(amount);
    });

    it("unpasue again and retest to make sure TRANSFER works again", async () => {
      await ARGH_Token.unpause();
      amount = "100000000000000000000";
      await ARGH_Token.transfer(receiver, amount);
    });
